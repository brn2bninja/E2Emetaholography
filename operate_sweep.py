...
ar = 15.0
num_meta = 1000
project="sweep_ar_" + str(ar) + "_num_" + str(num_meta)+ datetime.now().strftime("_%m%d_%H%M")
fwd_model = 'LSTM'
...
sweep_configuration = {
        ...
        "metric": {"goal": "minimize",
                   "name": "Total_loss"},
        "parameters": {
            "h_lr":     {"max": 1e-1,   "min": 1e-3,    'distribution': 'log_uniform_values'},
            "lw_lr":    {"max": 1e-1,   "min": 1e-3,    'distribution': 'log_uniform_values'},
            "mat_lr":   {"max": 1e-1,   "min": 1e-3,    'distribution': 'log_uniform_values'},
            "numP_lr":  {"max": 1e-1,   "min": 1e-3,    'distribution': 'log_uniform_values'},
            "p_lr":     {"max": 1e-1,   "min": 1e-3,    'distribution': 'log_uniform_values'},
            "rot_lr":   {"max": 2e-1,   "min": 7e-2,    'distribution': 'log_uniform_values'},
        },
        # "early_terminate": {
        #     "type": "hyperband",
        #     "s": 2,
